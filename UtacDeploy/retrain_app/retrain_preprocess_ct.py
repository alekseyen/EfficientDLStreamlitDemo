...
def get_feat_cols():
    feature = ['mean','std','min','max','qntl1','qntl3','median','rows','dow']
...
def transform(df_spc2,df_mc2,h,xmin,history,ct_index):
    ...
    for i in range (len(df_spc2)):
        ...
        date_index = datetime.strptime(ct_index[i], "%Y-%m-%d %H:%M:%S")
        ...
        #x = x.iloc[:,1:]
        y = [df_spc2.iloc[i].MEANX,df_spc2.iloc[i].dayofweek,df_spc2.index.values[i]]
...
def create_dataset(df_spc2,df_mc2,n,write,xmin,history,ct_index,task):
    X,y = transform(df_spc2=df_spc2,df_mc2=df_mc2,h=n,xmin=xmin,history=history,ct_index=ct_index)
    X,y = select_features(X,y,task,n)
    X,y,y_scaler,X_Scaler = normalize(X,y,write,task,n)

    return X,y,y_scaler,X_Scaler

def preprocess_dataset_ct(df_mc,df_spc,hours,task):
    ...
    write = True
    xmin = 2
    X,y,y_scaler,X_Scaler = create_dataset(df_spc2,df_mc2,n,write,xmin,history,ct_index,task)
